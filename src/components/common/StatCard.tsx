...
export const StatCard: React.FC<StatCardProps> = ({
  ...
  const chartHeight = compact ? 28 : 50;
  const valueVariant = compact ? 'h5' : 'h4';
  const contentSx = compact ? { p: 1.5 } : undefined;
        ...
        <Stack direction="column" sx={{ justifyContent: 'space-between', flexGrow: '1', gap: 1 }}>
          <Stack sx={{ justifyContent: 'space-between' }}>
            <Stack direction="row" sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
              <Typography variant={valueVariant} component="p">
                {value}
              </Typography>
              {/* Placeholder chip to mirror template spacing; hidden by default unless you want to show deltas */}
              <Chip size="small" color={labelColors[trend]} label="" sx={{ visibility: 'hidden' }} />
            </Stack>
            {interval && (
